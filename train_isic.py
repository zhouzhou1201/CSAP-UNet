...
def test(model, path):
    ...
    for s in ['val', 'test']:
        ...
        for i in range(test_loader.size):
            ...
            with torch.no_grad():
                _, _, res = model(image)
            loss = structure_loss(res, torch.tensor(gt).unsqueeze(0).unsqueeze(0).to(device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default='F:/Desktop/xxxx/xiaorong daima copy/flops/TransFuse-main/data', help='path to train dataset')
    parser.add_argument('--test_path', type=str,
                        default='F:/Desktop/xxxx/xiaorong daima copy/flops/TransFuse-main/data', help='path to test dataset')
    ...
    flops, ps = profile(model, (dummy_input,))
    print('flops: ', flops, 'ps: ', params)
    #print('flops: %.2f M, ps: %.2f M' % (flops / 1000000.0, ps / 1000000.0))
